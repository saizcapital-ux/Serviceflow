...
| `ai_coach.py` | Sends your trade stats to the Claude API → regenerates the AI Coach (`coach.js`) |
| `coach.js` | The AI Coach's analysis (generated) |
...
- **AI Coach** — an AI-written assessment of your trading: a grade, a candid
  ...
  `python ai_coach.py` (uses the Claude API).
...
## Refresh the AI coaching

The AI Coach panel is generated by Claude from your trade statistics. To
re-run it against your latest data:
...
It computes an aggregate profile of your trading (win rate, payoff math, edge
by hour/weekday/size, post-loss tilt, rule-based what-ifs), sends **only those
aggregates** to the Claude API, and writes `coach.js`. Reload the page to see
the new assessment.
